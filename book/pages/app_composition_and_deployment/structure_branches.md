...
| Deploy   | **Yes** - 手動 or 継続的       | ベースと、"production" (または dev、staging など) のイベントに対する反応として要求される変更 | **メモリリソース**の増量 - たとえばコンテナのクラッシュにより | `deploy-test`, `deploy-staging`, `deploy-prod` |
...
- ベースのタグ *v2* が切られ、アーティファクト (イメージなど) がビルドされる
